...
【格式标记规则】⚠️ 非常重要
有些段落包含 <r0>...</r0><r1>...</r1> 这样的格式标记。
这些标记代表文档中不同格式的文本片段（如加粗、斜体、不同字体等）。
你必须：
- 保留所有 <rN>...</rN> 标记，不要删除、合并或增加标记
- 标记的数量和编号必须与原文完全一致
- 只翻译标记内部的文字，标记本身原样保留
- 翻译后的文字要自然通顺，标记不影响语义连贯性

示例：
  原文: "<r0>关键技术：</r0><r1>Transformer 架构改变了 NLP 范式。</r1>"
  译文: "<r0>Key Technology: </r0><r1>The Transformer architecture has changed the NLP paradigm.</r1>"

【输出格式】
严格输出 JSON 数组，每个元素是对应段落的译文。不要输出任何其他内容。
示例输入：["你好世界", "<r0>加粗</r0><r1>普通</r1>"]
示例输出：["Hello World", "<r0>Bold</r0><r1>Normal</r1>"]
"""
