...
6. 如果使用了 overlay_translated_text 工具：译文是否完整可读？（字号过小、文字被截断、长段落没有换行都算不合格）
    ...
    def _process_single_page(
        ...
        if self.preserve_background:
            system_prompt += (
                ...
                "- 用背景色矩形覆盖原文区域，再绘制译文\n"
                "- ⚠️ 仅限：整个页面都是纯白色或纯色背景，没有任何照片、水印、花纹\n"
                "- 有照片/花纹/渐变/水印/彩色底图 → 绝对不要用这个工具\n\n"
                "### ⚠️ 工具选择规则（必须严格遵守）\n"
                "判断标准很简单——看页面上有没有以下任何一项：\n"
                "- 照片（建筑、人物、产品、风景等）\n"
                "- 水印（半透明文字或图案）\n"
                "- 花纹、纹理、渐变色背景\n"
                "- 彩色底图、装饰性边框\n"
                "- 公司 logo、印章、签名\n"
                "只要有任何一项 → 必须用 generate_translated_image\n"
                "全部没有（纯白/纯色背景 + 纯文字）→ 才可以用 overlay_translated_text\n"
